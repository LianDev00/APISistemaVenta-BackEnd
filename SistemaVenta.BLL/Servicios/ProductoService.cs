...
namespace SistemaVenta.BLL.Servicios
{
    public class ProductoService : IProductoService
    {
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                var productoEnconctrado = await _productoRepositorio.Obtener(p => p.IdProducto == id);

                if (productoEnconctrado == null)
                    throw new TaskCanceledException("El proudcto no existe");

                bool respuesta = await _productoRepositorio.Eliminar(productoEnconctrado);
